    ...
    case TypeSpecification_PR_structure:
{
    ...
    if (asnTypeSpec == NULL) {
        printf("asnTypeSpec is NULL!\n");
        break;
    }

    if (asnTypeSpec->choice.structure.components.list.array == NULL) {
        printf("Structure components array is NULL!\n");
        break;
    }
    ...
    if (typeSpec->typeSpec.structure.elements == NULL) {
        printf("Memory allocation for structure elements failed!\n");
        break;
    }

    for (int i = 0; i < elementCount; i++) {
        if (asnTypeSpec->choice.structure.components.list.array[i] == NULL) {
            printf("Component %d is NULL, skipping\n", i);
            continue;
        }

        if (asnTypeSpec->choice.structure.components.list.array[i]->componentName == NULL) {
            printf("ComponentName %d is NULL, skipping\n", i);
            continue;
        }
        ...
        if (asnTypeSpec->choice.structure.components.list.array[i]->componentType == NULL) {
            printf("ComponentType %d is NULL, skipping\n", i);
            typeSpec->typeSpec.structure.elements[i] = NULL;
            continue;
        }

        typeSpec->typeSpec.structure.elements[i] =
            createTypeSpecification(
                asnTypeSpec->choice.structure.components.list.array[i]->componentType);

        if (typeSpec->typeSpec.structure.elements[i] != NULL) {
            typeSpec->typeSpec.structure.elements[i]->name = name;
        } else {
            printf("createTypeSpecification failed for element %d\n", i);
            GLOBAL_FREEMEM(name);
        }
    }
}
